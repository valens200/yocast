...
const podcastSlice = createSlice({
  ...
  reducers: {
    initializePodCast: (
      ...
      const whatTobeModified = action.payload.key;
      switch (whatTobeModified) {
        ...
        case "longDescription":
          ...
          break;
        case "Add short description for product":
          state.podcast.shortDescription = action.payload.value.toString();
          break;
        default:
          state.podcast.status = state.podcast.status;
      }
    },
    setSelected: (state, action: PayloadAction<{ id: number }>) => {
      const selectedCategory = state.podcastsCategories.filter(
        (category) => category.id === action.payload.id
      )[0];
      selectedCategory.selected = true;
      const unselectedCategory = state.podcastsCategories.filter(
        (category) => category.id != action.payload.id
      );
