import React from 'react'
import { HiMenuAlt1 } from "react-icons/hi"
import { GrHomeRounded } from "react-icons/gr"
import { BsBag, BsMoon } from 'react-icons/bs'
import { HiOutlineSquares2X2 } from "react-icons/hi2"
import { GiSquare } from "react-icons/gi"
import { IoMoonOutline, IoSunnyOutline } from "react-icons/io5"
import { IoMdNotificationsOutline } from 'react-icons/io'
import { TbFocusCentered } from "react-icons/tb"
import { setShowSidebar } from '../../features/pageSlice'
import { setIsDarkMode } from '../../features/pageSlice'
import { useDispatch, useSelector } from 'react-redux'
import { useSelect } from '@mui/base'
import { RootState, store } from '../../store'
import postcss from 'postcss'
import Box from '@mui/material/Box';
import Modal from '@mui/material/Modal';
import { Avatar, Backdrop, Button, CircularProgress, FormControl } from '@material-ui/core'
import 'react-dropdown/style.css';
import axios from 'axios'
import { baseUrl } from '../../assets/staticAssets/data'
import BackDrop from './BackDrop'
import { setShowBackDrop } from '../../features/pageSlice'
import { useNavigate } from 'react-router-dom'
import SucessAlert from '../alerts/SucessAlert'
import ErrorAlert from '../alerts/ErrorAlert'
function Navbar(props: { name: string }) {
    const podcastCreatedSucessfully = useSelector((store: RootState) => store.page.podcastPostedSucessfully);
    const loggedInUser = useSelector((store: RootState) => store.user.loggedInUser);
    const navigate = useNavigate();
    const icons = [<HiOutlineSquares2X2 />,
    <TbFocusCentered />,
    <IoSunnyOutline />,
    <IoMdNotificationsOutline />];
    const dispatch = useDispatch();
    const isDarkMode = useSelector((store: RootState) => store.page.isDarkMode);
    const user = JSON.parse(localStorage.getItem("user")!)
    const getClass = (index: number) => {
        if (index == 3) {
            return "absolute text-[white]  bg-[#405189]   font-bold w-[20px] h-[20px]  text-center items-center flex justify-center p-2 -translate-y-9  text-center rounded-full text-[0.70rem]"
        } else if (index == 4) {
            return "absolute text-[white] bg-[red] h-[20%] font-bold w-[5%]  md:w-[1.3vw]     -translate-y-9 translate-x-[40%] text-center rounded-full text-[0.70rem]"
        } else {
            return "absolute  hidden -translate-y-10 translate-x-[100%]   text-[0.70rem]"
        }
    }
    const options = ["Admin", "settings", "Logout"];
    const style = {
        position: 'absolute' as 'absolute',
        top: '7%',
        left: '50%',
        // transform: 'translate(-50%, -50%)',
        width: 800,
        bgcolor: isDarkMode ? '#2a2f34' : 'white',
        boxShadow: 24,
        pt: 2,
        px: 4,
        pb: 3,
    };
    const [open, setOpen] = React.useState(false);
    const handleOpen = () => {
        setOpen(true);
    };
    const handleClose = () => {
        setOpen(false);
    };
    const config = {
        user: { token: user.token }
    }
    const authorization = {
        headers: { Authorization: `Bearer ${user.token.token}` }
    }
    const logout = () => {
        console.log(user.token.token)
        axios.post(baseUrl + "/user/signout",
            config,
            authorization
        ).then((response) => {
            handleClose();
            console.log(response);
            dispatch(setShowBackDrop('show'))
            setTimeout(() => {
                localStorage.removeItem("user");
                dispatch(setShowBackDrop('hide'));
                navigate("/")
            }, 1000)
            localStorage.removeItem("user")
        }).catch((error) => {
            console.log(error);
        })
    }
    return (
        <div className={isDarkMode === true ? 'w-[100%] text-white font-sans bg-[#2a2f34] h-[12vh]' : 'w-[100%] text-black font-sans   bg-white h-[100%]'}>
            <div className='h-[60%]  w-[100%]'>
                <div className='flex flex-row h-[100%] items-center  justify-between w-[98%] mx-auto' >
                    <Modal
                        open={open}
                        onClose={handleClose}
                    >
                        <Box sx={{ ...style, width: 400, borderRadius: "4px", height: 100, marginRight: "auto", marginLeft: "auto" }}>
                            <p className={isDarkMode ? "text-white font-poppins font-sans" : "text-black font-poppins font-sans"} id="child-modal-description">
                                Are you sure you want to logout?
                            </p>
                            <div className={isDarkMode ? 'flex text-white  font-poppins font-sans flex-row space-x-2 items-center mt-4 w-[100%]' : 'flex text-black  font-poppins font-sans flex-row space-x-2 items-center mt-4 w-[100%]'}>
                                <button className='bg-[] w-[40%]' onClick={handleClose}>No</button>
                                <button onClick={logout} className='bg-[#F065481A]  rounded  p-1 w-[20%]'>Yes</button>
                            </div>
                        </Box>
                    </Modal>
                    <div className='  w-[30%] md:w-[70%] flex h-[100%] space-x-4 items-center flex-row '>
                        <div className='flex text-[grey]     flex-row space-x-8'>
                            <HiMenuAlt1 onClick={() => dispatch(setShowSidebar("show"))} className='text-3xl md:hidden' />
                            <HiMenuAlt1 className='text-3xl md:block hidden' />
                        </div>
                        <div className='md:w-[30%] md:block hidden h-[55%]'>
                            <input type="text" className={isDarkMode ? 'bg-[#1a1d21] focus:outline-0 text-[0.80rem] rounded pl-3 h-[100%] w-[100%]' : 'bg-[#f3f3f9] focus:outline-0 text-[0.80rem] rounded pl-3 h-[100%] w-[100%]'} placeholder='search....' />
                        </div>
                    </div>
                    <BackDrop />
                    <div className='md:w-[30%] w-[70%] justify-end items-center flex-row flex'>
                        <div className=''>
                            <ul className='flex flex-row space-x-3'>
                                {icons.map((icon, index) => {
                                    if (index == 2) {
                                        if (isDarkMode) {
                                            return (
                                                <li onClick={() => dispatch(setIsDarkMode("white"))} className='text-[1.4rem]   hover:bg-[#405189] p-2 rounded-full  hover:cursor-pointer' key={index}>{icon} </li>
                                            )
                                        }
                                        return (
                                            <li onClick={() => dispatch(setIsDarkMode("dark"))} className='text-[1.4rem] hover:bg-[#405189] p-2  rounded-full hover:cursor-pointer' key={index}><BsMoon /></li>
                                        )
                                    }
                                    return (
                                        <li onClick={() => dispatch(setIsDarkMode("main"))} className='text-[1.4rem] hover:bg-[#405189] z-100 p-2 rounded-full hover:cursor-pointer' key={index}>{icon} <p className={getClass(index)}>23</p></li>

                                    )

                                })}
                            </ul>
                        </div>
                        <div className='flex bg-[#B3B6B7 ] h-[100%]  justify-between space-x-3  font-poppins  flex-row'>
                            <div className='h-[100%] w-[30%] md:w-[20%]'>
                                <Avatar alt="Remy Sharp" src="data:image/jpeg;base64," />
                            </div>
                            <div className='h-[100%] font-poppins font-sans md:block hidden w-[50%]'>
                                <h1 className='text-[0.90rem]'>{loggedInUser.names}</h1>
                                <p onClick={handleOpen} className='text-[grey] text-[0.80rem] hover:font-bold hover:cursor-pointer'>Logout</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div className={isDarkMode ? 'bg-[#212529] sticky  top-0 w-[100%] h-[40%]' : 'bg-white border border-x-0 border-b-0 sticky  top-0 w-[100%] h-[40%]'}>
                <div className='flex font-poppins font-sans flex-row justify-between items-center  h-[100%] w-[97%] mx-auto'>
                    <div className='w-[90%]'>
                        <h1 className={podcastCreatedSucessfully ? 'font-bold  font-poppins  hidden font-sans uppercase' : 'font-bold  font-poppins font-sans uppercase'}>{props.name}</h1>
                    </div>
                    <div className='flex text-[0.90rem]  font-poppins font-sans flex-row w-[10%] justify-between'>
                        {/* <p className='md:text-white text-[#7c7f90]'>Yocast</p> */}
                        <p className='text-[grey] md:block hidden  font-bold-0 uppercase'>admin</p>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Navbar